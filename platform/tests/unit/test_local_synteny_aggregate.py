...
from genomelens.analysis.execution.handlers.multi_local import (
...
from genomelens.contracts.summaries import ChildRunRecord
...
def _job(tmp_path: Path, pair_id: str, target_name: str, query_bed: Path, subject_bed: Path) -> ChildRunRecord:
...
def test_multi_local_aggregate_scopes_gene_ids_per_species(tmp_path: Path) -> None:
    ...
    request = SyntenyExecutionRequest(
        reference=_prepared("query", query_bed),
        target=_prepared("subject", subject_bed),
