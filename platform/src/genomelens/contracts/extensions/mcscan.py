...
from genomelens.utils.json import _optional_int
...
class McscanSummaryExtension:
    ...
    这些字段不再直接挂在 `RunSummary` 上，而是通过 `extensions` 注入，
    ...
    child_run_count: int | None = None  # 子任务数量
    ...
    def to_dict(self) -> dict[str, object]:
        """转成可合并进 RunSummary.extensions 的字典"""
        ...
        if self.child_run_count is not None:
            data["pairwise_job_count"] = self.child_run_count
    ...
    def from_dict(cls, data: dict[str, object]) -> McscanSummaryExtension:
        """从 RunSummary.extensions 的原始字典还原"""
        ...
        return cls(
            ...
            child_run_count=_optional_int(data.get("pairwise_job_count")),
