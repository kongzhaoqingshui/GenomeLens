...
def task_scope(
    ...
    *,
    ...
    """在上下文中自动记录 task 开始、结束和失败"""

    started = time.perf_counter()
    task_started(logger, task_id=task_id, step=step, context=context)
    try:
        yield
    except Exception as exc:
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        task_failed(logger, task_id=task_id, step=step, elapsed_ms=elapsed_ms, error=exc, context=context)
        raise

    elapsed_ms = int((time.perf_counter() - started) * 1000)
    task_finished(logger, task_id=task_id, step=step, elapsed_ms=elapsed_ms, context=context)
