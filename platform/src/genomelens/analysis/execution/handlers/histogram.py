...
from genomelens.analysis.planning.models import HistogramExecutionRequest
...
from genomelens.validation.execution_requests import validate_histogram_request
...
def _run_histogram(
    ...
    with task_scope(logger, task_id=request.task_id, step="write_manifest", context={"manifest": str(layout.manifest)}):
        manifest = JcviManifestBuilder().build_histogram_manifest(request)
