# gljcvi-pairwise — 双物种共线性基础分析子模块插件

## 概述

`gljcvi-pairwise` 是 GenomeLens 在 HAIant（智然体）平台上的**双物种共线性基础分析**可编排子模块插件。它适合在两个基因组之间先建立最基础的同源与共线性关系，为后续点图、共线性图、核型图和局部共线性分析提供核心中间结果。

该子模块会完成双物种同源搜索、锚点扫描与共线性区块识别，并输出 blast table、anchors、simple、blocks 等结果；当开启 `emit_ortholog` 时，会额外产出双向 ortholog（同源基因对）目录。它最适合用于“先算清楚两个基因组之间有哪些稳定共线性片段，再决定后面怎么可视化或做候选位点分析”的场景。
运行完成后，通常会得到后续各类双物种图件都可复用的一组基础共线性中间结果。

本目录是 `gljcvi-pairwise` 插件包内容：
...
| `emit_ortholog` | bool | 否 | `false` | 是否在共线性结果之外额外产出双向 ortholog 目录（由 catalog 后端生成） |
...
2. 该子模块的真实调用凭证是 `output/submodule_request.json`；最终结果主要是 anchors、blocks、simple 等中间产物及其相关图件；开启 `emit_ortholog` 时还包含双向 ortholog 目录。
