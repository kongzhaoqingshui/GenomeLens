...
运行完成后，通常会得到一组可检索的直系同源对应表与相关中间结果，便于后续功能映射和跨物种基因对照。
...
| `input_dir` | dir | 是 | — | 双物种输入目录；每个物种应按同名前缀成对提供 `BED + CDS/PEP` 或 `GFF/GTF + genome FASTA`，插件会自动识别文件对并建立直系同源识别输入 |
...
插件会把双物种输入、同源识别参数与输出设置统一写入 `output/submodule_request.json`，再通过平台 `analyze run` 执行。`GenomeLens_Path` 未设置时读取 `GENOMELENS_EXE` 环境变量。
...
2. 该子模块的真实调用凭证是 `output/submodule_request.json`；最终结果主要是 anchors / ortholog 等中间产物目录。
