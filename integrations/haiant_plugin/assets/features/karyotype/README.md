...
This lightweight HAIant plugin corresponds to the ``analyze mcscan jcvi graphics_karyotype`` automatic flow.

It writes ``output/genomelens_request.json`` with ``workflow = graphics_karyotype`` and then calls the external GenomeLens executable:
