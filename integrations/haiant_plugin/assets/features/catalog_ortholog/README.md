...
├── run.log
└── results/
    └── speciesA.speciesB.ortholog    # 双向直系同源目录
```

- `genomelens_request.json`：可重放的请求 JSON。
- `run.log`：运行日志。
- `results/speciesA.speciesB.ortholog`：双向最佳匹配目录。具体文件名取决于 JCVI 对两个物种名的命名约定，通常为 `<speciesA>.<speciesB>.ortholog`。

目录文件内容一般为两列基因 ID，表示互为最佳匹配的同源基因对，例如：
