...
This lightweight HAIant plugin corresponds to the ``analyze mcscan jcvi catalog_ortholog`` automatic flow.

It writes ``output/genomelens_request.json`` with ``workflow = catalog_ortholog`` and then calls the external GenomeLens executable:
