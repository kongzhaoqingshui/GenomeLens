...
平台最新架构只承认两类公开任务协议：`WorkflowRequest`（仅 `synteny`）和 `SubmoduleRequest`（9 个子模块）。  
...
| `app/submodules/lightweight/gljcvi-pairwise.zip` | 可编排子模块 | `output/submodule_request.json` | 双物种共线性基础结果（`emit_ortholog=true` 时附带双向直系同源目录） |
...
### `gljcvi-pairwise`（`jcvi.pairwise`）

- 端口：`species_pair`（由 `input_dir` 映射）。
- 参数：`align_soft`、`dbtype`、`cscore`、`dist`、`iter`、`min_block_size`、`emit_ortholog`。
- `emit_ortholog=true` 时附带输出双向 ortholog 目录（原 `catalog_ortholog` 模块已并入本模块）。
