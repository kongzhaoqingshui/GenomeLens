...
| 版本 | 主题 | 目标时间 | 关键内容 |
|------|------|----------|----------|
| v0.9.0 | 体验与输入优化 | 已就绪 | CLI 分页帮助、`analyze mcscan jcvi` 子命令、局部共线性目标基因高亮、自动目录输入混用模式、文档刷新 |
| v0.9.x | JCVI 增强、可观测性与插件化 | 后续迭代 | 在 v1.0 前补齐：run.log、analyze run JSON、智然体插件、JCVI 子任务 CLI、多物种局部共线性图、出图自动优化 |
