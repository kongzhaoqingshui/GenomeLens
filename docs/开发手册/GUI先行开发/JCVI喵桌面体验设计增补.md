...
图标是一个小猫头拿着透镜，透镜中显示 `JCVI`。

需借鉴用户给出的参考图的「极简小猫头记忆点」，但必须有明显变化：

- 猫头不使用原参考的红色方形脸，改为冰蓝/白色半透明脸部。
- 脸型可保留圆角几何感，但轮廓更像柔和盾形或水滴圆角方块，耳朵更尖更轻。
- 眼睛从两个点改为短竖线或微笑眼；可加一小段鼻梁/胡须，不照搬参考。
- 透镜为右下或正前方主要识别物：圆形镜片、短柄，镜片内写 `JCVI`。
- 色彩：主色 `--ice-500`，浅底 `--ice-50`，描边 `#0F172A` 或深色模式下 `#E0F2FE`。
- 图标需能在 16/32/64/128 px 下辨认；小尺寸下可隐藏 `JCVI` 文字，只保留镜片高光。

### 2.2 交付形态

B 先用前端内联 SVG 或组件实现，不新增依赖，不引入未经确认的外部图片资源。后续需要安装包图标时再由 GUI lead 统一导出 `.ico` / `.png` / `tauri.conf.json` 图标资产。
...
解决进入软件后等待后端能力探测时的卡顿感。启动后立即展示中心动画，后端 `get_version()`、模板/schema 预热、环境检查等可并行进行。
...
3. 透镜中以打字机效果打印 `JCVI`。
4. 猫头 + 透镜组成最终应用图标，周围出现 2-3 条淡冰蓝基因连线。
...
加载完成后，不进入传统 landing hero，而是进入「中心能力环」。中心是 JCVI喵图标，周围扩散出能力入口。能力入口围绕中心圆形/椭圆环绕，但位置有轻微不规则感，避免机械仪表盘。
...
- 实现 `JcviMeowIcon` 组件，先用内联 SVG/CSS，不新增依赖。
