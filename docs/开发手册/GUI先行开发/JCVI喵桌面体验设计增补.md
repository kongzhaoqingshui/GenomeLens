...
B 先用前端内联 SVG 或组件实现，不新增依赖，不引入未经确认的外部图片资源。安装包与窗口图标由 GUI lead 统一导出 `.ico` / `.png` / `tauri.conf.json` 图标资产。
...
除 JCVI喵品牌 logo 外，其他能力、工具、设置类图标统一使用 Nieobie/Game-Icon-Pack 的 SVG 资产。该图标包采用 CC0 1.0 Universal，允许修改和商用；项目内保留来源说明即可，不需要运行时联网加载。
