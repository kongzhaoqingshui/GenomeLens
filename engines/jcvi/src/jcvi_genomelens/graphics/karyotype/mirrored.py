...
def _synteny_graphics():
    ...
    return Shade, ymid_offset


def _default_label_va(va: str) -> str:
    if va == "bottom":
        return "top"
    if va == "top":
        return "bottom"
...
class LayoutLine:
    ...
    def __init__(self, row, delimiter=",", generank=True):
        ...
        if len(args) == 9:
            ...
            self.label_va = _default_label_va(self.va)
...
class ShadeManager:
    ...
    def draw_blocks(self, ax, blocks, atrack, btrack, samearc: str | None, heightpad=0):
        ...
        shade_cls, ymid_offset_func = _synteny_graphics()
        for a, b, c, d, _, _, highlight in blocks:
            ...
            ymid_pad = ymid_offset_func(samearc)
            if heightpad:
