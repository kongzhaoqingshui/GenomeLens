...
def _render_overlay(
    ...
    *,
    ...
    for index, item in enumerate(series):
        ...
        ax.hist(item.values, bins=bin_edges, alpha=alpha, label=item.label, color=color, edgecolor="#1f3b4d")
    if len(series) > 1:
...
def _render_facets(
    ...
    *,
    ...
    for axis, item in zip(axes.flat, series, strict=False):
        axis.hist(item.values, bins=bin_edges, color=fill, edgecolor="#1f3b4d")
